...
getLang();
  const getTranslates = async () => {
    let response = await RestService.getTranslates(limit, skip,undefined);
    translates = response["translates"];
    console.log(response,"translates")
...
<div class="flex flex-wrap mt-4 h-screen relative">
  <div class="w-full mb-12 px-2 lg:px-4 ">
    <button
      ...
      type="button"
      on:click={() => {
        navigate("/panel/create-translate");
      }}
    >
      <i class="fa fa-plus" />
      {$Translate("Add")}
                ...
                <tr>
                  ...
                  >
                    <button
                      ...
                      type="button"
                      on:click={navigate(
                        `/panel/update-translate/${translate._id.toString()}`
                      )}
                    >
                      {$Translate("Edit")}
                    </button>
                    <button
                      on:click={() =>
                        deleteTranslateApprove(translate._id.toString())}
                      class="bg-white text-blue-600 hover:bg-blue-600 hover:text-white border border-blue-600 rounded font-bold  text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
                      type="button"
                    >
                      {$Translate("Delete")}
