<script>
  ...
  values.map((v) => {
    if (v.defaultValue) {
      event[v.key] = { value: v.defaultValue };
    } else {
      event[v.key] = {};
    }
  });
  ...
  const addEvent = async () => {
    ...
    let response = await RestService.addEvent(data);
    if (response["status"]) {
    ...
    >
      ...
      <div class="flex-auto px-4 lg:px-10 py-10 pt-0">
        <div class="flex flex-wrap my-4">
            ...
            <div class="relative w-full mb-3">
              ...
              {#if boxOffices}
                {#each boxOffices as boxOffice}
                  <button
                    on:click={() => {
                      if (event.boxOffices.value.indexOf(boxOffice._id) == -1) {
          ...
          <div class="w-full lg:w-6/12 px-4">
            <div class="relative w-full mb-3 flex flex-row gap-2 items-center">
              <div>
                <label
                  class="block  text-blueGray-600 text-xs font-bold mb-2"
                  for="rectangleBanner"
                ...
                <Date bind:value={event.eventDate.value} placeholder="Tarih" />
              </div>
              <div>
                <label
                  class="block  text-blueGray-600 text-xs font-bold mb-2"
