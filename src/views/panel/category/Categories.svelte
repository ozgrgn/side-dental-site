<script>
  ...
  const getCategories = async () => {
    let response = await RestService.getCategories(limit, skip);
...
<div class="flex flex-wrap mt-4 h-screen relative">
  <div class="w-full mb-12 px-2 lg:px-4 ">
    <button
      ...
      type="button"
      on:click={() => {
        navigate("/panel/create-category");
      }}
    >
      <i class="fa fa-plus" />
      {$Translate("Add")}
      ...
      <div class="rounded-t mb-0 px-4 py-3 border-0">
        <div class="flex flex-wrap items-center">
          <div class="relative w-full px-4 max-w-full flex-grow flex-1">
            <h3 class="font-semibold text-lg text-blueGray-700">Category</h3>
          </div>
        </div>
      </div>
                ...
                <tr>
                  ...
                  >
                    <button
                      ...
                      type="button"
                      on:click={navigate(
                        `/panel/update-category/${category._id.toString()}`
                      )}
                    >
                      {$Translate("Edit")}
                    </button>
                    <button
                      on:click={() =>
                        deleteCategoryApprove(category._id.toString())}
                      class="bg-white text-blue-600 hover:bg-blue-600 hover:text-white border border-blue-600 rounded font-bold  text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
                      type="button"
                    >
                      {$Translate("Delete")}
