<script>
  ...
  const getPermissionGroups = async () => {
    let response = await RestService.getPermissionGroups(limit, skip);
    permissionGroups = response["permissionGroups"];
...
<div class="flex flex-wrap mt-4 h-screen relative">
  <div class="w-full mb-12 px-2 lg:px-4 ">
    <button
      ...
      type="button"
      on:click={() => {
        navigate("/panel/create-permissionGroup");
      }}
    >
      <i class="fa fa-plus" />
      {$Translate("Add")}
                ...
                <tr>
                  ...
                  >
                    <button
                      ...
                      type="button"
                      on:click={navigate(
                        `/panel/update-permissionGroup/${permissionGroup._id.toString()}`
                      )}
                    >
                      {$Translate("Edit")}
                    </button>
