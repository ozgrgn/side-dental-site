<script>
  ...
  const deleteShowApprove = (showId) => {
    modal.set(
      bind(Alert, {
  ...
  const getShows = async () => {
    let response = await RestService.getShows(limit, skip);
  ...
  const deleteShow = async (showId) => {
    let response = await RestService.deleteShow(showId);
    if (response["status"]) {
                ...
                <tr>
                  ...
                  >
                    <button
                      ...
                      type="button"
                      on:click={navigate(
                        `/panel/update-show/${show._id.toString()}`
                      )}
                    >
                      {$Translate("Edit")}
                    </button>
                    <button
                      on:click={() => deleteShowApprove(show._id.toString())}
                      class="bg-white text-blue-600 hover:bg-blue-600 hover:text-white border border-blue-600 rounded font-bold  text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
                      type="button"
                    >
                      {$Translate("Delete")}
