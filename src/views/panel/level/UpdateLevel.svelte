<script>
  ...
  const deleteLevelApprove = (levelId) => {
    modal.set(
      bind(Alert, {
  ...
  const updateLevel = async () => {
    let editedLevel = {};
    values.map((v) => {
      editedLevel[v.key] = level[v.key].value;
    });

    let response = await RestService.updateLevel(level._id, editedLevel);
    if (response["status"]) {
  ...
  const getLevel = async () => {
    let response = await RestService.getLevel($params.levelId);

    if (response["status"]) {
      values.map((v) => {
        if (v.customValue) {
          response["level"][v.key] = {
            value: response["level"][v.key][v.customValue],
          };
        } else {
          response["level"][v.key] = {
            value: response["level"][v.key],
          };
        }
      });
      level = {
  ...
  const deleteLevel = async (levelId) => {
    let response = await RestService.deleteLevel(levelId);
    if (response["status"]) {
  ...
  <div class="w-full mb-12 px-2 lg:px-4 ">
    <div class="flex justify-between">
      ...
      <button
        class="bg-white text-blue-600 hover:text-red-700 mb-2 border rounded font-bold text-xs px-4 py-2 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 "
        type="button"
        on:click={() => deleteLevelApprove($params.levelId)}
      >
        <i class="fa fa-trash" />
        Sil
      </button>
