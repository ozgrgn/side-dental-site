<script>
  import Svg from "../../../assets/svg.json";
</script>

<div class="pt-20">
  <h4 class="text-center text-5xl font-semibold text-dark-300 tracking-tight">
    <span class="text-light-300">Where is</span> Side Shine Dental?
  </h4>
  <div class="flex">
    <img class="w-3/5 shadow-xl pt-5" src="assets/img/map.png" alt="" />
    <div
      class="-ml-20 shadow-xl bg-dark-300 h-96 w-2/6 flex flex-col justify-center my-auto px-10"
    >
      <div class="flex flex-col pl-3 text-white">
        <span class="text-2xl font-semibold ">Working Hours</span>
        <p class="">Check out DentiCare’s Office hours to plan your visit.</p>
        <div class="flex flex-col ">
          <div class=" border-b border-white/30 pt-10 flex justify-between">
            <div class="w-20">Monday</div>
            <div class="w-28 text-white/60">08:00 - 18:00</div>
            <a class="" href="/">
              <div
                class="bg-white w-16 h-6 mb-2 text-center flex flex-col justify-center rounded-md text-sm text-light-300"
              >
                <div class="flex justify-center">
                  Book <svg
                    class="h-3 w-3 mt-[0.2rem] ml-1"
                    fill="#bc74a5"
                    viewBox="0 0 122.88 122.88"
                    ><style type="text/css">
                      .st0 {
                        fill-rule: evenodd;
                        clip-rule: evenodd;
                      }
                    </style>

                    >
                    {@html Svg.clock}
                  </svg>
                </div>
              </div>
            </a>
          </div>
          <div class=" border-b border-white/30 pt-2 flex justify-between">
            <div class="w-20">Monday</div>
            <div class="w-28 text-white/60">08:00 - 18:00</div>
            <a class="" href="/">
              <div
                class="bg-white w-16 h-6 mb-2 text-center flex flex-col justify-center rounded-md text-sm text-light-300"
              >
                <div class="flex justify-center">
                  Book <svg
                    class="h-3 w-3 mt-[0.2rem] ml-1"
                    fill="#bc74a5"
                    viewBox="0 0 122.88 122.88"
                    ><style type="text/css">
                      .st0 {
                        fill-rule: evenodd;
                        clip-rule: evenodd;
                      }
                    </style>

                    >
                    {@html Svg.clock}
                  </svg>
                </div>
              </div>
            </a>
          </div>
          <div class=" border-b border-white/30 pt-2 flex justify-between">
            <div class="w-20">Monday</div>
            <div class="w-28 text-white/60">08:00 - 18:00</div>
            <a class="" href="/">
              <div
                class="bg-white w-16 h-6 mb-2 text-center flex flex-col justify-center rounded-md text-sm text-light-300"
              >
                <div class="flex justify-center">
                  Book <svg
                    class="h-3 w-3 mt-[0.2rem] ml-1"
                    fill="#bc74a5"
                    viewBox="0 0 122.88 122.88"
                    ><style type="text/css">
                      .st0 {
                        fill-rule: evenodd;
                        clip-rule: evenodd;
                      }
                    </style>

                    >
                    {@html Svg.clock}
                  </svg>
                </div>
              </div>
            </a>
          </div>
          <div class=" border-b border-white/30 pt-2 flex justify-between">
            <div class="w-20">Monday</div>
            <div class="w-28 text-white/60">08:00 - 18:00</div>
            <a class="" href="/">
              <div
                class="bg-white w-16 h-6 mb-2 text-center flex flex-col justify-center rounded-md text-sm text-light-300"
              >
                <div class="flex justify-center">
                  Book <svg
                    class="h-3 w-3 mt-[0.2rem] ml-1"
                    fill="#bc74a5"
                    viewBox="0 0 122.88 122.88"
                    ><style type="text/css">
                      .st0 {
                        fill-rule: evenodd;
                        clip-rule: evenodd;
                      }
                    </style>

                    >
                    {@html Svg.clock}
                  </svg>
                </div>
              </div>
            </a>
          </div>
          <div class=" border-b border-white/30 pt-2 flex justify-between">
            <div class="w-20">Monday</div>
            <div class="w-28 text-white/60">08:00 - 18:00</div>
            <a class="" href="/">
              <div
                class="bg-white w-16 h-6 mb-2 text-center flex flex-col justify-center rounded-md text-sm text-light-300"
              >
                <div class="flex justify-center">
                  Book <svg
                    class="h-3 w-3 mt-[0.2rem] ml-1"
                    fill="#bc74a5"
                    viewBox="0 0 122.88 122.88"
                    ><style type="text/css">
                      .st0 {
                        fill-rule: evenodd;
                        clip-rule: evenodd;
                      }
                    </style>

                    >
                    {@html Svg.clock}
                  </svg>
                </div>
              </div>
            </a>
          </div>
          <div class="pt-2 flex justify-between">
            <div class="w-20">Monday</div>
            <div class="w-28 text-white/60">08:00 - 18:00</div>
            <a class="" href="/">
              <div
                class="bg-white w-16 h-6 mb-2 text-center flex flex-col justify-center rounded-md text-sm text-light-300"
              >
                <div class="flex justify-center">
                  Book <svg
                    class="h-3 w-3 mt-[0.2rem] ml-1"
                    fill="#bc74a5"
                    viewBox="0 0 122.88 122.88"
                    ><style type="text/css">
                      .st0 {
                        fill-rule: evenodd;
                        clip-rule: evenodd;
                      }
                    </style>

                    >
                    {@html Svg.clock}
                  </svg>
                </div>
              </div>
            </a>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>
